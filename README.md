...
| dataExtractor    | (response:any) => any[]                            | An optional function that convert the desiered result from server to an array. if server response is an array by itself leave we don't need to pass this function.                   |
| loadingElement   | ReactNode                                          | an Optional ReactNode element if we want to show user as loading state                                                                                                               |
| errorElement     | ReactNode                                          | an Optional ReactNode element if we want to show user as error state                                                                                                                 |
